...
// Get user's connected Instagram profile (authenticated endpoint)
router.get("/my-profile", authenticateToken, async (req, res) => {
  try {
    ...
    // Use the user's stored access token to fetch their profile
    const profileUrl = `${req.protocol}://${req.get(
      "host"
    )}/api/instagram/profile?accessToken=${user.instagram.accessToken}`;
...
router.get("/status", (req, res) => {
  res.json({
    ...
    endpoints: [
      ...
      "GET /api/instagram/my-profile (authenticated)",
